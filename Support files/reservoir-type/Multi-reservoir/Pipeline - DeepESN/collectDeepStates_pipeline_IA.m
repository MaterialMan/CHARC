function[statesExt] = collectDeepStates_pipeline_IA(genotype,inputSequence,config)    
        ...
        for n = 2:length(inputSequence(:,1))          
            if i == 1
                states{i}(n,:) = feval(genotype.reservoirActivationFunction,((genotype.esnMinor(i).inputWeights*genotype.esnMinor(i).inputScaling)*([genotype.esnMinor(i).inputShift inputSequence(n,:)])')+ genotype.connectWeights{i,i}*states{i}(n-1,:)'); %n-1
                 ...
                 states{i}(n,:) = feval(genotype.reservoirActivationFunction,((genotype.esnMinor(i).inputWeights*genotype.esnMinor(i).inputScaling)*([inputSequence(n,:) states{i-1}(n,:)])')+ genotype.connectWeights{i,i}*states{i}(n-1,:)'); %n-1
            end
        end
