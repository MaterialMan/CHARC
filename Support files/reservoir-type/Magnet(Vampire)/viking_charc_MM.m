...
function viking_charc_MM(ID, network_size)
...
config.res_type = 'MM';
config.num_nodes = network_size;

fprintf('Reservoir: %s, Size: %d, run: %d \n',config.res_type,network_size,ID)
...
config.pop_size = 3;                       % initail population size. Note: this will generally bias the search to elitism (small) or diversity (large)
config.total_gens = 2;                    % number of generations to evolve
...
%save data
saveDatabase(database_history,database,quality,tests,config,network_size);
...
function saveDatabase(database_history,database,quality,test,config,network_size)
config.figure_array =[];

if test == 0 
    save(strcat('/mnt/lustre/users/md596/working-branch/Ucnc retests/',config.res_type,'/Results/_run',num2str(test),'_networkSize_',network_size,'.mat'),...
