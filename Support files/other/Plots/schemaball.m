function h = schemaball(r, lbls, ccolor, ncolor, figHandle)

% Notes: use >> schemaball(x,[],[0,0,1;1 1 1]) for red and blue
...
if nargin < 3 || isempty(ccolor);
    %Center white
    ccolor = hsv2rgb([[linspace(.8333, .95, N); linspace(1,0,N); flip(linspace(1,0.7,N))],...
                      [linspace(.03, .1666, N); linspace(0,1,N); flip(linspace(0.7,1,N))]]');
                  
%Center black
%                   ccolor = hsv2rgb([[linspace(.8333, .95, N); zeros(1, N); linspace(1,0,N)],...
%                       [linspace(.03, .1666, N); zeros(1, N); linspace(0,1,N)]]');
    ...
    ccolor = hsv2rgb([[repmat(ccolor(1,1),1,N); linspace(1,0,N); flip(linspace(ccolor(1,end),0.7,N))],...
        [repmat(ccolor(2,1),1,N); linspace(0,1,N); flip(linspace(0.7,ccolor(2,end),N))]]');
end
