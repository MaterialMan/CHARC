function [OUTPUT_DATA]=mars(filename,stop_time,speed,options)
%
...
%-- Preliminary check
if nargin==0, fprintf('The first argument (filename) is mandatory. Program aborted.\n'); return; end
EXP.Filename=filename;
%------
%-- Read experiment data from user function (run user function with second argument = true)
clear global
eval(['[Command , EXP]=' EXP.Filename '(EXP,true);']);
%------
%-- Check and set input argument
if nargin>=2, if ~isempty(stop_time), EXP.Stop_time=stop_time; end, end
if nargin>=3, if ~isempty(speed), EXP.Animation.Playback_speed=speed; end, end
...
for T1=0:EXP.Sampling_time:EXP.Stop_time
    ...
    if (EXP.Animation.Enable)
        EXP=plot_robots(EXP);        % draw animation at time T1
    ...
    eval(['[Command, EXP]=' EXP.Filename '(EXP,false);']); %-- Run user-defined function
...
switch(EXP.Filename)
    %strcmp(EXP.Filename,'Demo_Obstacles') || strcmp(EXP.Filename,'Demo_Sensor')
    case 'Demo_Obstacles'
    subplot(2,2,3)
    bar([EXP.Map.Obstacle_distance.Min_dist])
    xlabel('Sim step')
    ylabel('Ob proximity')
    
    subplot(2,2,4)
    plot(EXP.History.Pose(:,3))
    xlabel('Sim step')
    ...
    case 'Demo_Sensor'
        if EXP.Time >= 1
%             subplot(2,2,3)
%             bar(EXP.Agent(1).Sensor.Measured_distance(end))
%             xlabel('Sim step')
%             ylabel('Sensor proximity')
