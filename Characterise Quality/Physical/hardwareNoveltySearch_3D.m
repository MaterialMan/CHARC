...
config.material = 'LED64'; %label material being used

% Evo settings
config.popSize =5; %200
config.deme = round(config.popSize*0.2); %sub-species increase diversity (0.2)
config.numMutate = 0.2;   %0.2 
config.recRate = 0.7; %1
config.numTests = 5; %5
config.totalGens = 2000; %2000
...
config.p_min = 3; %3
config.p_min_check = 200; %200

%general params
config.genPrint = 1; %200
...
config.input_range = 32; %how many inputs to be used at one time. No more than 32. 16 is good default.
...
for tests = 1:config.numTests
    ...
    for gen = 2:config.totalGens
        ...
        pop_metrics = [kernel_rank;gen_rank;MC]';
        pop_metrics_ext = [kernel_rank;gen_rank; MC; Mapping']';
        ...
        if  dist > config.p_min || rand < 0.01 %storeError(tests,eval,loser)
